...
- [ ] 7. Generate and execute CAPTURE_TEST_DATA query
...
1. **Identify Path**: Determine the correct path for the new policy under `/ce`. For example: `/ce/ca/aws/ec2/instance-excessive-volume-attachments`.
...
**IMPORTANT: Always use CAPTURE_TEST_DATA to generate test data. Never write test-data.json manually.**

The environment has `bq` command properly configured for BigQuery access.

**Workflow:**

1. **Generate Capture Query** (no testData section needed yet):
   ```bash
   repo-manager policies generate CAPTURE_TEST_DATA /path/to/wip.logic.yaml
   ```
   This creates a `.sql` file in the `.generated` directory (e.g., `.generated/capture_test_data.sql`).

2. **Execute the Query with bq**:
   ```bash
   # Execute the generated SQL and save results directly to test-data.json
   bq query --format=json --use_legacy_sql=false < /path/to/policy/.generated/capture_test_data.sql > /path/to/policy/test-data.json
   ```

3. **Add testData section to wip.logic.yaml**:
...
- **`repo-manager policies generate <kind> <id>`**: Generates a BigQuery script for a policy.
  ...
  - **`<kind>`**: The type of script to generate. Valid values are:
    ...
    - `CAPTURE_TEST_DATA`: Generates a SQL query to capture real data from BigQuery for creating test-data.json. Execute the generated SQL with `bq query` and save results to test-data.json.
...
### Executing CAPTURE_TEST_DATA

Always use bq to execute the generated SQL and capture test data:

```bash
# Step 1: Generate the capture query (wip.logic.yaml should NOT have testData section yet)
repo-manager policies generate CAPTURE_TEST_DATA /path/to/wip.logic.yaml

# Step 2: Execute with bq and save to test-data.json
bq query --format=json --use_legacy_sql=false < .generated/capture_test_data.sql > test-data.json

# Step 3: Add testData section to wip.logic.yaml now that you have actual test data
# Add this to wip.logic.yaml:
#   testData:
#     - file: "test-data.json"

# Step 4: Edit test-data.json to add expectedResult for each test case
```
