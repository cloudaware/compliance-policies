...
7. Repeat steps 4–6 to create event subscriptions for other Amazon RDS security groups in the current AWS Region, if applicable.
...
1. Create an Amazon SNS topic to receive RDS event notifications:
    ...
    aws sns create-topic \
        --name {{rds-securitygroup-event-notifications}}
    ```
...
5. Create an Amazon RDS event subscription for DB security group **configuration change** and **failure** events:
    ...
    aws rds create-event-subscription \
        ...
        --source-ids {{db-security-group-id}} \
        --enabled
    ```
...
7. Repeat steps 1–6 for additional RDS security groups in the same AWS Region, if required.
