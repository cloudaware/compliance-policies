...
This document outlines the structure of the policy repository and provides best practices for developing effective and maintainable Compliance Engine policies.
**Remember that policy development is not just about writing "code"; it's about declaratively defining compliance rules in a clear, understandable, and maintainable way.**

## Repository Structure

You can host any files in your repository, not just Compliance Engine entities. 
When developing policies, we encourage storing supplementary materials like screenshots, internal notes, and downloaded external documentation to provide context and intent for future maintenance and updates.

Key root-level files and directories relative to `root.yaml` file, include:

- `root.yaml`: Marker of the repository root directory.
- `ce`: Root folder for all policies, containing subfolders:
  ...
  - `ce/{yourOrg}`: Your organization's policies
...
- `types/types.json`: File that contains all available types in the organization (updated with any call to `repo-manager types import`)
- `tmp`: Directory to store temporary files, like exports or task-related files
- `flags`: Contains tagged entity lists generated by `repo-manager docs` (useful for tracking WIP policies, policies missing documentation, tests, etc.)
- `.ca`: Default location of repository level override of `~/.ca` directory, where `repo-manager` stores configuration and authorization data
...
> Paths vs IDs:
>
> - IDs in Compliance Engine behave similarly to file paths in Unix-like systems
> - Compliance Engine prefers absolute IDs for pointing to entities like Types, Framework Sections, etc. (`/types/CA10__CaAwsInstance__c`)
> - Relative IDs usually used for Test Data (e.g., `test-data.json`)
> - Repository root is referenced as `/`
> - Folder paths omit trailing slashes (e.g., `/ce/ca/aws/ec2` instead of `/ce/ca/aws/ec2/`)
> - Most `repo-manager` commands accept:
>   - Absolute IDs (e.g., `/ce/ca/aws/ec2/instance-detailed-monitoring`)
>   - Relative IDs (e.g., `ce/ca/aws/ec2/instance-detailed-monitoring`)
>   - Absolute OS-paths (e.g., `C:\Users\user\compliance-engine\ce\ca\aws\ec2\instance-detailed-monitoring`)
>   - Relative OS-paths (e.g., `ce\ca\aws\ec2\instance-detailed-monitoring`)
> - This document will paths relative to the repository root (folder containing `root.yaml`) and absolute IDs for clarity. 
...
| ID               | Path            | Contextual Name   | Full Name         |
|------------------|-----------------|-------------------|-------------------|
| `/ce`            | `ce`            | Compliance Engine | Compliance Engine |
| `/ce/ca`         | `ce/ca`         | CloudAware        | CloudAware        |
| `/ce/ca/aws`     | `ce/ca/aws`     | AWS               | AWS               |
| `/ce/ca/aws/ec2` | `ce/ca/aws/ec2` | AWS EC2           | EC2               |
...
- **Root directories**: Use predefined names with specific purposes:
  ...
  - `schema`, `tmp`, `flags`: Follow exact naming for tooling compatibility
...
- **Special directories**: Reserved names with specific behaviors:
  ...
  - `.ca`, `.docusaurus`, `.vscode`: Configuration directories
...
- **Alternate locations**: Reference via absolute ID (e.g., `/ce/my-company/tagging/test-data.json`) or relative ID (e.g., `../shared-test-data.json`)
...
While the files in the repository are human-readable, they lack functionality for developers to easily navigate between different entities. 
Tasks such as the following are nearly impossible without generated documentation:
...
The primary command for documentation generation is `repo-manager docs generate`. This command analyzes the entire repository and produces `*.gen.md` files in the same directories as descriptor files and other entities. 
Documentation is generated for both permanent files (like `policy.yaml`) and temporary files (excluded via default `/.gitignore`, such as `{logicName}.test-results.yaml`).

`repo-manager docs generate` also creates a `flags` directory containing flagged entities for repository management and continuous improvement, including:

- `flags/logic-without-test-data.gen.md` - Logic files lacking [test data](#test-data)
- `flags/logic-test-results-missing.gen.md` - Logic files with missing test results (typically indicating policy failures after running `repo-manager policies test all`)
- `flags/policy-with-todo.md.gen.md` - Policies containing TODO files
- `flags/policy-without-description.md.gen` - Policies missing description files
- `flags/policy-without-remediation.md.gen` - Policies missing remediation guidance
